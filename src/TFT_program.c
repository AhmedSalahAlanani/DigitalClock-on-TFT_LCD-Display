...
void HTFT_voidInit (void)
{
	...
	MTimer3_voidSetBusyWait(TFT_INIT_DELAY_3);
...
void HTFT_voidDisplayImage (const u16* Copy_pu16Image)
{
	...
	voidWriteData 	 (TFT_WIDTH_START&(0xFF00));
	voidWriteData	 (TFT_WIDTH_START&(0x00FF));
	voidWriteData	 (TFT_WIDTH_END&(0xFF00));
	voidWriteData	 (TFT_WIDTH_END&(0x00FF));

	/*	SET ROW ADDRESS	*/
	voidWriteCommand (TFT_COMMAND_RASET);
	voidWriteData	 (TFT_HEIGHT_START&(0xFF00));
	voidWriteData	 (TFT_HEIGHT_START&(0x00FF));
	voidWriteData	 (TFT_HEIGHT_END&(0xFF00));
	voidWriteData	 (TFT_HEIGHT_END&(0x00FF));
...
void HTFT_voidDrawPixel (u8 Copy_u8X, u8 Copy_u8Y, u16 Copy_u16Colour)
{
	/*	SET COLUMN ADDRESS	*/
	voidWriteCommand (TFT_COMMAND_CASET);
	voidWriteData 	 (Copy_u8X&(0xFF00));
	voidWriteData	 (Copy_u8X&(0x00FF));
	voidWriteData	 (Copy_u8X&(0xFF00));
	voidWriteData	 (Copy_u8X&(0x00FF));

	/*	SET ROW ADDRESS	*/
	voidWriteCommand (TFT_COMMAND_RASET);
	voidWriteData	 (Copy_u8Y&(0xFF00));
	voidWriteData	 (Copy_u8Y&(0x00FF));
	voidWriteData	 (Copy_u8Y&(0xFF00));
	voidWriteData	 (Copy_u8Y&(0x00FF));
...
{
	/*	SET BOUNDARIES	*/
	if((Copy_u8X1 + Copy_u8X2 - 1) >= TFT_WIDTH)
	{
		Copy_u8X2 = TFT_WIDTH  - Copy_u8X1;
	}

	if((Copy_u8Y1 + Copy_u8Y2 - 1) >= TFT_HEIGHT)
	{
		Copy_u8Y2 = TFT_HEIGHT - Copy_u8Y1;
	}

	/*	SET COLUMN ADDRESS	*/
	voidWriteCommand (TFT_COMMAND_CASET);
	voidWriteData 	 (Copy_u8X1&(0xFF00));
	voidWriteData	 (Copy_u8X1&(0x00FF));
	voidWriteData	 (Copy_u8X2&(0xFF00));
	voidWriteData	 (Copy_u8X2&(0x00FF));

	/*	SET ROW ADDRESS	*/
	voidWriteCommand (TFT_COMMAND_RASET);
	voidWriteData	 (Copy_u8Y1&(0xFF00));
	voidWriteData	 (Copy_u8Y1&(0x00FF));
	voidWriteData	 (Copy_u8Y2&(0xFF00));
	voidWriteData	 (Copy_u8Y2&(0x00FF));
...
void HTFT_voidDrawString (u8* Copy_pu8String, u8 Copy_u8X, u8 Copy_u8Y, u16 Copy_u16TextColour, u16 Copy_u16BGColour, u8 Copy_u8Size)
{
	while(*Copy_pu8String)
	{
		if(*Copy_pu8String <= 9)
		{
			HTFT_voidDrawCharacter ( (*Copy_pu8String)+'0', Copy_u8X, Copy_u8Y, Copy_u16TextColour, Copy_u16BGColour, Copy_u8Size);
